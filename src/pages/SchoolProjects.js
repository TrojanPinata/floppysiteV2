      ...
      return (
         <div className="App">
            <div className="proj">
               ...
               <h3>Hardware Sorting with FPro Core - FPGA Design in VHDL</h3>
               <p>This project was the final lab for my final and highest degree VHDL course. The goal was to generate a set of random numbers and sort them via a hardware core we designed and simultaneously sort them with a basic software insertion sort and compare the time it took to perform each - and the speedup gained from a dedicated hardware module on the chip. The results were expected, with the hardware sort being considerably faster. The learning experience from this assignment was designing hardware which would interact with the provided CPU and the code we wrote to run on both. This involved writing drivers and core modules in C++ which would then compile with Vitis and execute on the MMIO core.</p>
               <h3>MOSFET Amplifier Design Project - Linear Electronics</h3>
               <p>This was one of two projects for my linear electronics course which entailed designing a amplifier using NMOS in order to satisfy a pretty strict set of criteria. This is one of those projects where explaining too much might be a problem, so I will pretty shortly sya that it was a fun project which mostly revolved around cascading amplifiers and very weird results in pspice.</p>
               <h3>Lock, Semephores, and Syscalls in OS161 - Operating Systems</h3>
               <p>This entry is not exactly one single project, but a compilation of projects I did in OS161. The projects for this operating systems class were aimed at giving the students the chance to implement locks, semaphores, etc. in order to solve small problems and games which were provided in the OS. The final project for this class was to implement Unix standard syscalls and all of the problems that brings with implementing them on a MIPS CPU. The first project went extremely well for my partner and I, but the second was not so fun. I really enjoyed this class, and it is my favorite computer science class I have attended while in university.</p>
               </Fade>
            </div>
            <Footer />
         </div>
